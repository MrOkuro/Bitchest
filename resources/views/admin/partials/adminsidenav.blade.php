<div class="col-md-4 SideNav">
    <ul class="list-group">
    	
        <li class="list-group-item"><a href="{{   route('admin.index') }}"><i class="fa fa-user" aria-hidden="true"></i>&nbsp;Afficher liste des clients</a></li>
        <li class="list-group-item"><a href="{{   route('admin.users.create') }}"><i class="fas fa-plus" aria-hidden="true"></i>&nbsp;Ajouter nouveaux clients</a></li>
        <li class="list-group-item"><a href=""><i class="fa fa-user" aria-hidden="true"></i>&nbsp;Gerer son profil</a></li>
        
    </ul>
    <ul class="list-group">
        <li class="list-group-item"><a href="{{ route('admin.crypto.index') }}"><i class="fa fa-list" aria-hidden="true"></i>&nbsp; Afficher cours des crypto monnaies</a></li>
    </ul>
    <ul class="list-group">
        
            <li class="list-group-item">Mon solde en Euros : <strong> €</strong></li>

    </ul>
</div>